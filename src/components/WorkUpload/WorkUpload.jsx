...
const WorkUpload = () => {
    ...
    useEffect(()=>{
        const fetchWorks = async()=>{
          ...
          const res = await fetch('/api/getAllWork',{
            method:'GET',
            headers:{
  ...
  const handleDelete = async(workId)=>{
    try {
      setLoading(true)
 const res = await fetch(`/api/deletework/${workId}`,{
  method:'DELETE',
  headers:{
    'Content-Type':'application/json'
  }
    ...
    <Box sx={{background:'linear-gradient(to right, #141e30, #243b55)',height:'100dvh',overflow:'auto',width:'100%',display:'flex',flexDirection:'column',alignItems:'center' }}>
        ...
        <Container sx={{pt:'1rem'}}>
            ...
            <Box sx={{display:'flex',justifyContent:'flex-end',mt:'1rem'}}>
                ...
                <Typography fontSize='14px' mt='0.2rem' fontWeight='bold'>Add Work</Typography>    
                    
                </Button>
            </Box>
         
         <Box sx={{background:'#fff',borderRadius:'5px',p:'1rem',mt:'1rem'}}>
         <Typography fontSize='25px' fontWeight='bold'>Works</Typography>
         </Box>
        ...
        <AddWork dialog={open} onClose={handleClose} />
     {open1 && <EditWork dialog={open1} onClose={handleClose1} id={workId} /> }   
    </Box>     
  )
}
